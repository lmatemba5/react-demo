...
const Board = ({ barOptions, withdrawnOptions }) => {
  ...
  const showProgress = () => {
    let percent =
      data?.prg?.checked === 0
        ? 0
        : Number((data?.prg?.checked / data?.prg?.clients) * 100).toFixed(0);
    percent = isNaN(percent) ? 0 : percent;

    return (
      <div
        className={
          "text-center text-white progress-bar-striped bg-" +
          (percent < 26
            ? "danger"
            : percent < 51
            ? "info"
    ...
    <div className="expandonprint">
      <Row style={{ fontSize: "14px" }}>
        <Col item="true" xs={12} md={3} className="mt-4 expandonprint">
          <Link className="nav-link" to="#">
            <div
              className="card p-0 border-danger rounded-4"
              style={{
                ...
                <div className="row d-flex align-items-center">
                  ...
                  <div className="col-4 d-flex justify-content-end">
                    <ListCheck
                      size={50}
                      color="white"
                      className="rounded-4 bg-primary p-3"
                      style={{ boxShadow: "2px 2px 4px lightgrey" }}
                    />
                  </div>
                </div>
              </div>
      ...
      <Row>
        <Col xs={12} md={12} lg={12} className="expandonprint mt-4">
          <Row className="mb-2">
            <Col>
              <label className="fw-bold fs-5 text-secondary">
                Work Progress
              </label>
            </Col>
          </Row>
          <div
            className="card bg-white rounded-4 border-0 px-3"
            style={{ maxHeight: "280px", boxShadow: "1px 1px 5px lightgrey" }}
          >
            <div className="row mb-3">
              ...
              <div className="col-md-3 mb-1 text-center d-flex flex-column justify-content-center">
                ...
                <span>
                  {data?.prg?.checked} of {data?.prg?.clients}
                </span>
              </div>
              ...
              <div className="col-md-6 col-sm-12 px-4 align-items-center d-flex flex-column justify-content-center">
                ...
                <div
                  className="progress mb-3 w-100 mt-1 "
                  style={{ height: "16px" }}
                >
                  {showProgress()}
                </div>
      ...
      <Row className="mb-4">
        <Col xs={12} md={6} className="mt-4 expandonprint">
          ...
          <div
            className="card bg-white rounded-4 p-0  border-0"
            style={{ boxShadow: "2px 2px 4px lightgrey" }}
          >
