    ...
    return (
        ...
        <Container fluid className="mb-4" id="content">
            ...
            <Row className="px-2 mb-4">
                <Col xs={12} md={12} lg={12} className="px-2 py-4 d-flex justify-content-between">
                    <span className="px-2 fs-3 fw-bold text-secondary">Dashboard</span>
                    <ReactToPrint
                        trigger={() => <Button className="badge btn-sm btn btn-danger rounded-4 px-3">Export PDF</Button>}
            ...
            {
                role === 1 ? 
                    <DashboardContent bearer={{headers:{Authorization: 'Bearer '+token()}}} id={getRole()} ref={(el) => (componentRef = el)}/>
                : (
                    role === 3 ?
                        <FiDashboard bearer={{headers:{Authorization: 'Bearer '+token()}}} id={getRole()} ref={(el) => (componentRef = el)}/>
                    : 
                    <FODashboard bearer={{headers:{Authorization: 'Bearer '+token()}}} id={getRole()} ref={(el) => (componentRef = el)}/>
                )
